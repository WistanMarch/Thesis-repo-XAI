...
def do_interpretability(config_args_list, target):
    ...
    for step, val_data in enumerate(val_loader):
        ...
        for model_to_load in config_args_list:
            ...
            val_output = do_pred(models=model, x=val_inputs, sigmoid=False, softmax=True)
            
            ### BP BEGINS
            
            # Extraction of max output
            target_pred = val_output[0, target[step]].requires_grad_()
            
            # Do backpropagation to get the derivative of the output based on the image
            attribution_map = torch.autograd.grad(target_pred, val_inputs, grad_outputs=torch.ones_like(target_pred))[0].squeeze().cpu().detach().numpy()
            ...
            ni_img = nib.Nifti1Image(attribution_map, affine=val_data['input_meta_dict']['affine'].squeeze())
            nib.save(ni_img, pathResults + val_data['patient_id'][0] + '_' + model_name + ".nii")
            
            ### BP ENDS
            
            # Prepare for average attribution
            attrs += attribution_map
            nb_models += 1
