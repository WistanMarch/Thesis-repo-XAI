...
# Base paths
pathRoot = './'
pathResults = pathRoot + 'Results/Raw_attrs/Captum_IntegratedGradients'
...
def do_interpretability(config_args_list, target):
    ...
    for step, val_data in enumerate(val_loader):
        ...
        attrs_black = np.zeros_like(val_inputs.squeeze().cpu().detach().numpy(), dtype=np.float64)
        attrs_white = np.zeros_like(val_inputs.squeeze().cpu().detach().numpy(), dtype=np.float64)
        attrs_mean = np.zeros_like(val_inputs.squeeze().cpu().detach().numpy(), dtype=np.float64)
        model_iter = 0
    
        # # Baselines are both black (min of input) and white (max of input) images
        # baseline_black = torch.full(val_inputs.shape, torch.min(val_inputs))
        # baseline_white = torch.full(val_inputs.shape, torch.max(val_inputs))
        
        # Baselines are both black (0) and white (1) images
        baseline_black = torch.zeros(size=val_inputs.shape, device=device, requires_grad=True)
        baseline_white = torch.ones(size=val_inputs.shape, device=device, requires_grad=True)
        ...
        for model_to_load in config_args_list:
            ...
            # IntegratedGradients init
            ig = IntegratedGradients(do_pred, multiply_by_inputs=True)
            
            # Computes integrated gradients
            attr_IGB = ig.attribute(inputs=val_inputs, baselines=baseline_black, target=target[step], additional_forward_args=(model, False, True), n_steps=200, method='gausslegendre', internal_batch_size=10, return_convergence_delta=False)
            attr_IGW = ig.attribute(inputs=val_inputs, baselines=baseline_white, target=target[step], additional_forward_args=(model, False, True), n_steps=200, method='gausslegendre', internal_batch_size=10, return_convergence_delta=False)
    
            attr_IGB_numpy = attr_IGB.squeeze().detach().cpu().numpy()
            attr_IGW_numpy = attr_IGW.squeeze().detach().cpu().numpy()
            attr_IGBW_numpy = np.mean([attr_IGB_numpy, attr_IGW_numpy], axis=0)
            ...
            ni_img_black = nib.Nifti1Image(attr_IGB_numpy, affine=val_data['input_meta_dict']['affine'].squeeze())
            nib.save(ni_img_black, pathResults + '(Black)/' + val_data['patient_id'][0] + '_' + model_name + ".nii")
            ni_img_white = nib.Nifti1Image(attr_IGW_numpy, affine=val_data['input_meta_dict']['affine'].squeeze())
            nib.save(ni_img_white, pathResults + '(White)/' + val_data['patient_id'][0] + '_' + model_name + ".nii")
            ni_img_mean = nib.Nifti1Image(attr_IGBW_numpy, affine=val_data['input_meta_dict']['affine'].squeeze())
            nib.save(ni_img_mean, pathResults + '(BlackWhite)/' + val_data['patient_id'][0] + '_' + model_name + ".nii")
            
            # # Prepare for average attribution
            attrs_black += attr_IGB_numpy
            attrs_white += attr_IGW_numpy
            attrs_mean += attr_IGBW_numpy
        ...
        attrs_black /= model_iter
        attrs_white /= model_iter
        attrs_mean /= model_iter
        # Save average
        ni_attrs_black = nib.Nifti1Image(attrs_black, affine=val_data['input_meta_dict']['affine'].squeeze())
        nib.save(ni_attrs_black, pathResults + '(Black)/' + val_data['patient_id'][0] + ".nii")
        ni_attrs_white = nib.Nifti1Image(attrs_white, affine=val_data['input_meta_dict']['affine'].squeeze())
        nib.save(ni_attrs_white, pathResults + '(White)/' + val_data['patient_id'][0] + ".nii")
        ni_attrs_mean = nib.Nifti1Image(attrs_mean, affine=val_data['input_meta_dict']['affine'].squeeze())
        nib.save(ni_attrs_mean, pathResults + '(BlackWhite)/' + val_data['patient_id'][0] + ".nii")
