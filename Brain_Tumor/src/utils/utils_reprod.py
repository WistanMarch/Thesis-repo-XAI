...
from tta_reprod import simple_tta
...
def dir_path(string):
    ...
    :param string: String of the path.
    :return: String if existed, else raise NotADirectoryError.
    """
...
def get_split_deterministic(all_keys, fold=0, num_splits=5, random_state=12345, y_stratified=None):
    """
    Splits a list of patient identifiers (or numbers) into num_splits folds and returns the split for fold.
    :param all_keys: keys used for splitting
    :param fold: fold number
    :param num_splits: number of splits
    :param random_state: seed number
    :param y_stratified: if y used for stratification of data
    :return: train keys, valid keys
    """
    train_keys, test_keys = np.array([]), np.array([])
    all_keys_sorted = np.sort(list(all_keys))

    if y_stratified is not None:
        assert len(all_keys_sorted) == len(y_stratified), "Keys and y stratified need to be the same length"
        splits = StratifiedKFold(n_splits=num_splits, shuffle=True, random_state=random_state)
        for i, (train_idx, test_idx) in enumerate(splits.split(all_keys_sorted, y_stratified)):
            if i == fold:
...
def create_patient_train_test_split(patient_dict, fold, num_splits, seed=12345, y_stratified=None):
    """
    From the patient dict create the train and val patient ID.
    :param patient_dict: patient dict
    :param fold: Number of folds
    :param num_splits: Number of split
    :param seed: Random state
    :param y_stratified: stratify or not during splitting
    :return train_patient, val_patient
    """
    # create train, val dict
    train_patient, val_patient = get_split_deterministic([*patient_dict], fold=fold, num_splits=num_splits,
                                                         random_state=seed, y_stratified=y_stratified)
    logging.info(f'\nTraining: {round(len(train_patient) / len([*patient_dict]) * 100)}%'
                 f'\nValidation: {round(len(val_patient) / len([*patient_dict]) * 100)}%')
    return train_patient, val_patient
    ...
    if None not in (label_path, patient_col, label_col):
        for patient_id in list(patient_dict.keys()):
            ...
            for lab in labels_cols_input:
                ...
                elif not isinstance(label_value, (float, int, np.integer, np.float)):
...
def shape_to_model(data, min_shape=None):
    ...
    zeropad_shape = np.ceil(np.divide(datashape, 16)).astype(np.int) * 16
...
def do_pred(x, models, do_tta=False, sigmoid=False, softmax=False):
    if not isinstance(models, (tuple, list)):
        models = [models]

    todos_tta = simple_tta(x)

    outputs_models = []
    for model in models:
        outputs_tta = []
        if do_tta:
            for im, _ in todos_tta:
                out = apply_activation(model(im), sigmoid, softmax)
