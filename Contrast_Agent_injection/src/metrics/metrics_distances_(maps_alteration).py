...
def main():
    ...
    for arch in networks:
        ...
        base_methods = [x.replace('SGSQ+', '').replace('SG+', '').replace('ImgReg+', '') for x in methods]
        df_all['Base_method'] = base_methods
        alterations = [x.replace(base_methods[idx], '').replace('+', '') for idx,x in enumerate(methods)]
        df_all['Modification'] = ['None' if x == '' else x for x in alterations]
        ...
        # Init figure
        figVS, axVS = plt.subplots(nrows=1, ncols=2, figsize=(20, 7), squeeze=False)
        
        # Plot DSC vs Fidelity
        sns.scatterplot(data=df_all, x="DSC", y="Fidelity", hue="Modification", style="Base_method", ax=axVS[0][0], legend='full', s=400).legend(loc='center right', title='DSC vs Fidelity', fontsize='15', title_fontsize='18', bbox_to_anchor= (1.45,0.5))
        # Plot ROC vs Fidelity
        sns.scatterplot(data=df_all, x="ROC", y="Fidelity", hue="Modification", style="Base_method", ax=axVS[0][1], legend='full', s=400).legend(loc='center right', title='ROC vs Fidelity', fontsize='15', title_fontsize='18', bbox_to_anchor= (1.45,0.5))
        
        # Save overall results file
        pathSave = pathArch + 'ExpectationVsFaithfulness/'
        os.makedirs(pathSave, exist_ok=True)
        df_all.to_csv(pathSave + 'all_distances_maps_alteration.csv')
        
        # Save curves
        figVS.tight_layout()
        figVS.savefig(pathSave + 'Expect_vs_faith_maps_alteration.tiff')
